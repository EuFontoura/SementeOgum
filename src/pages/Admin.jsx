...
export default function Admin() {
  ...
  async function handleDeleteQuestion(questionId) {
    ...
    try {
      const { name, day } = currentExam;
      await deleteDoc(
        doc(db, "exams", name, "days", day, "questions", questionId)
      );
  ...
  async function handlePublishProva() {
    ...
    if (questions.length === 0)
      return alert("Adicione questões antes de publicar");
    try {
      const { name, day } = currentExam;
      const existingProva = provas.find(
        (p) => p.name === name && p.day === day
      );
      ...
      if (!existingProva) {
        setProvas((prev) => [
          ...prev,
          { id: provaRef.id, name, day, questionCount: questions.length },
        ]);
      }
      setCurrentExam(null);
    } catch (err) {
    ...
    return (
      <div className="p-10 text-center">
        ...
        <p className="text-sm text-gray-600 mt-2">
          Se você deveria ter acesso, peça a um administrador para adicionar seu
          UID à lista de admins.
        </p>
      </div>
    );
  }
                ...
                <div>
                  ...
                  {r.finishedAt && (
                    <p className="text-sm text-gray-600">
                      Data:{" "}
                      {new Date(r.finishedAt.seconds * 1000).toLocaleString()}
                    </p>
                  )}
                </div>
                <button
    ...
    return (
      <div className="p-6 max-w-lg mx-auto">
        ...
        <h2 className="text-2xl font-bold mb-2">
          Detalhes de {detailedResult.name}
        </h2>
        <p className="text-xl mb-4">
          Pontuação:{" "}
          <span className="font-bold">
            {detailedResult.score} / {detailedResult.total}
          </span>
        </p>
        <div className="mb-6">
  ...
  if (currentExam) {
    ...
    return (
      <div className="p-6 max-w-5xl mx-auto">
        ...
        {Object.entries(SUGGESTED_QUESTIONS_BY_DAY[day] || {}).map(
          ([t, count]) => {
            const current = countByTema[t] || 0;
            return (
              <p key={t} className={current > count ? "text-red-500" : ""}>
          ...
          <div className="mb-2">
            ...
            {previewURL && (
              <img
                src={previewURL}
                alt="Preview"
                className="my-2 max-w-xs rounded"
              />
            )}
          </div>
          <button
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-8 p-4 border rounded-lg shadow">
        ...
        <p className="text-sm text-gray-600 mb-2">
          Digite o nome e o dia da prova que deseja criar ou editar as questões.
        </p>
        <div className="flex flex-col md:flex-row gap-2">
          ...
          <button
            className="bg-blue-500 text-white px-4 py-2 rounded"
            onClick={() => {
              if (!newExamName || !newExamDay)
                return alert("Preencha o nome e o dia.");
      ...
      <div>
        <h2 className="text-xl font-bold mb-4">
          Provas Publicadas (Visíveis aos Alunos)
        </h2>
        {provas.length === 0 ? (
